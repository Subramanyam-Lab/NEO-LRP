"""
Solves CVRP using VROOM solver.
Computes routes for each open depot's assigned customers,
"""
...
def _extract_routes_robust(sol, assigned_ids):
    """Extract routes from VROOM solution, mapping back to original customer IDs."""
    routes_out = []
...
class dataCvrp:
    ...
    def runVROOM(self):
        """Runs VROOM once per open depot, aggregating costs and routes."""
    ...
    def _build_vroom_matrix(self, locations, rc_cal_index):
        """Build symmetric distance matrix for VROOM."""
        n = len(locations)
    ...
    def solve_demo(
        ...
        vc = vroom.VehicleCosts(fixed=1000)
