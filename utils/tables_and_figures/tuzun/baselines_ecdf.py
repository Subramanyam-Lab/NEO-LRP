...
Tuzun-Burke benchmark. Produces plots for both optimization gap and runtime,
...
latex_file = "tuzun_sota_table_updated.tex"

with open(latex_file, "r") as f:
    lines = f.readlines()

instance_pattern = re.compile(
    r"^\s*(?P<instance>\d{6})\s*&\s*"
    r"(?P<BKS>[\d\.]+)\s*&\s*"
...
def plot_ecdf(df, value_col, filename, xlabel, ylabel, legend_bool):
    ...
    if filename.endswith("gap_tuzun_new"):
        ax.text(0.5, -0.28, r"(a)", transform=ax.transAxes,
                ha="center", va="center", fontsize=22)

    if filename.endswith("runtime_tuzun_new"):
...
plot_ecdf(
    df=df_ecdf,
    value_col="Time",
    filename="ecdf_runtime_tuzun_new",
