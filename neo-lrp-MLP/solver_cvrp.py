"""
VRP Solver Module for MLP-based Neural Embedded Optimization

This module provides functionality to solve Capacitated Vehicle Routing Problem (CVRP)
instances using VRPSolverEasy for exact solutions.
"""

import math
import getopt
import sys
import os
import argparse
import time
from dataparse import create_data
from VRPSolverEasy.src import solver
import logging
import os
import sys
from datetime import datetime


class dataCvrp:
    """
    Contains all data for CVRP problem and provides methods to solve VRP instances.
    
    This class handles the integration between the neural embedded optimization
    results and the VRPSolverEasy solver for obtaining exact VRP solutions.
    """

    def __init__(self, ans, flp_ass):
        """
        Initialize CVRP solver with problem data and facility assignment results.
        
        Args:
            ans (list): Parsed problem data from dataparse module
            flp_ass (list): Facility location problem assignment results
        """
        
        self.nb_customers = ans[0]
        self.nb_open_depot=list(flp_ass[1].keys())
        self.coord=flp_ass[2]
        self.vehicle_capacity = ans[4]
        self.rc_cal_index=ans[9]
        self.flp_cost=flp_ass[0]
        self.vrp_cost=0
        self.tot_routes=0
        self.flp_ass=flp_ass
        self.message=[]
        self.variable_cost=[]
        self.num_routes=[]
        self.actual_routes=[]
        self.solve_time=[]
        self.model_exec_time=0
        

    def runVRPeasy(self):
        """
        Run VRPSolverEasy to solve VRP instances for all open depots.
        
        Returns:
            list: Results containing total cost, VRP costs, actual costs, routes, and execution times
        """
        for j in self.nb_open_depot:
            ass_cord=self.coord[j]
            cust_demands = self.flp_ass[4][j]
            cust=self.flp_ass[1][j]
            vrp_data=self.create_data_model(ass_cord,self.nb_open_depot,cust_demands,self.vehicle_capacity,self.nb_customers,cust,self.rc_cal_index)
            cost,m,message,routes,exec_time=self.solve_files_in_directory(vrp_data,cust)
            self.actual_routes.append(routes)
            self.variable_cost.append(cost)
            self.num_routes.append(m)
            self.solve_time.append(exec_time)
            self.model_exec_time+=exec_time
            self.message.append(message)
            self.vrp_cost+=cost
            print(cost,m)
            self.tot_routes+=m
            print("sequence of routes:",routes)
            logging.info(f"Sequence of routes is {routes}")
        print("Final VRP variable cost:",self.vrp_cost)
        logging.info(f"Final VRP variable cost is {self.vrp_cost}")
        print("Number of routes:",self.tot_routes)
        logging.info(f"Number of routes is {self.tot_routes}")
        

        total_lrp_cost = self.flp_cost + self.vrp_cost
        total_vrp_cost = self.vrp_cost

        print("Total LRP cost",total_lrp_cost)

        logging.info(f"Final VRP cost is {total_lrp_cost}")
        
        return total_lrp_cost,total_vrp_cost,self.variable_cost,self.num_routes,self.message,self.solve_time,self.actual_routes,self.model_exec_time


    def compute_euclidean_distance(self,x_i, y_i, x_j, y_j,rc_cal_index):
        """Compute the euclidean distance between 2 points from graph"""
        if rc_cal_index==0:
            return int(100*(math.hypot(((x_i - x_j)),((y_i - y_j)))))
        else:
            return int(math.hypot(((x_i - x_j)),((y_i - y_j))))


    def create_data_model(self,all_cord,op_dep,cust_dem,veh_cap,customer_no,cust,rc_cal_index):
        """Stores the data for the problem."""
        data = {}
        data['locations'] =all_cord
        data['open_depot']=op_dep
        data["nb_cust"]=customer_no
        data['demands'] = cust_dem
        data['vehicle_capacities'] = veh_cap
        data['depot'] = 0
        data['cust_cor']=all_cord[1:]
        data['depot_cor']=all_cord[0]
        data['assigned_cust']=cust
        data['rc_cal_index']=rc_cal_index
        return data

    def solve_demo(self, data, cust, F=1000, time_resolution=3600, solver_name_input="CLP", solver_path=""):

        # Model formulation
        model = solver.Model()

        logging.info(f"Number of Max customers {data['nb_cust']}")
        logging.info(f"Vehicle Capacity {data['vehicle_capacities'][0]}")
        # Add vehicle type
        model.add_vehicle_type(id=1,
                            start_point_id=0,
                            end_point_id=0,
                            max_number=data['nb_cust'],
                            capacity=data['vehicle_capacities'][0],
                            fixed_cost = 1000,
                            var_cost_dist=1 # we dont have any variable cost for distance. It is incorporated from the euclidean distance only?
                            )
        # Add depot
        model.add_depot(id=0)

        # Add all customers
        for i in range(1,len(data["locations"])):

            model.add_customer(id=i,
                            demand=data["demands"][i]                           
                            )

        
        # Compute links between depot and other points
        for i in range(1,len(data['locations'])):
            dist = self.compute_euclidean_distance(float(data['locations'][i][0]),
                                            float(data['locations'][i][1]),
                                            float(data['locations'][0][0]),
                                            float(data['locations'][0][1]),
                                            data['rc_cal_index']
                                            )

            model.add_link(start_point_id=0,
                        end_point_id=i,
                        distance=dist
                        )
        

        # Compute links between points
        
        for i in range(1,len(data['locations'])):
            j=i+1
            for j in range(i + 1, len(data['locations'])):
                dist = self.compute_euclidean_distance(float(data['locations'][i][0]),
                                            float(data['locations'][i][1]),
                                            float(data['locations'][j][0]),
                                            float(data['locations'][j][1]),
                                            data['rc_cal_index']
                                            )
                model.add_link(start_point_id= i,
                            end_point_id=j,
                            distance=dist
                            )

        
        # Set parameters
        model.set_parameters(time_limit=time_resolution,
                            solver_name=solver_name_input)

        if solver_name_input == "CPLEX":
            model.parameters.cplex_path = solver_path
            model.set_parameters(solver_name="CPLEX", time_limit=time_resolution)


        logging.info(f"Model is {model}")
        
        # Solve model
        st=datetime.now()
        try:
            model.solve()
            ed=datetime.now()
            exec_time=(ed-st).total_seconds()
            
            if model.solution.is_defined:
                cost = model.solution.value
                m = len(model.solution.routes)
                message = model.message
                routes = []
                for route in model.solution.routes:
                    routes.append({
                        "Ids": list(map(str, route.point_ids))
                    })
            else:
                cost = 0
                m = 0
                message = "No solution found"
                routes = []
                
        except Exception as e:
            ed=datetime.now()
            exec_time=(ed-st).total_seconds()
            
            # Fallback to simple heuristic if VRPSolverEasy fails (e.g., BaPCod not installed)
            logging.warning(f"VRPSolverEasy failed: {e}. Using simple heuristic fallback.")
            
            try:
                # Simple heuristic: Nearest Neighbor with capacity constraints
                locations = data['locations']
                demands = data['demands']
                capacity = data['vehicle_capacities'][0]
                
                unvisited = list(range(1, len(locations)))  # Skip depot (0)
                routes = []
                total_cost = 0
                
                while unvisited:
                    route = [0]  # Start from depot
                    current_load = 0
                    current_pos = 0
                    
                    while unvisited and current_load < capacity:
                        # Find nearest unvisited customer that fits capacity
                        nearest_dist = float('inf')
                        nearest_customer = None
                        
                        for customer in unvisited:
                            if current_load + demands[customer] <= capacity:
                                dist = self.compute_euclidean_distance(
                                    float(locations[current_pos][0]),
                                    float(locations[current_pos][1]),
                                    float(locations[customer][0]),
                                    float(locations[customer][1]),
                                    data['rc_cal_index']
                                )
                                if dist < nearest_dist:
                                    nearest_dist = dist
                                    nearest_customer = customer
                        
                        if nearest_customer is None:
                            break
                        
                        # Add customer to route
                        route.append(nearest_customer)
                        current_load += demands[nearest_customer]
                        total_cost += nearest_dist
                        current_pos = nearest_customer
                        unvisited.remove(nearest_customer)
                    
                    # Return to depot
                    return_dist = self.compute_euclidean_distance(
                        float(locations[current_pos][0]),
                        float(locations[current_pos][1]),
                        float(locations[0][0]),
                        float(locations[0][1]),
                        data['rc_cal_index']
                    )
                    total_cost += return_dist
                    route.append(0)
                    routes.append(route)
                
                # Format routes for compatibility
                formatted_routes = []
                for route in routes:
                    formatted_routes.append({
                        "Ids": list(map(str, route))
                    })
                
                cost = total_cost
                m = len(routes)
                message = f"Heuristic fallback (VRPSolverEasy error: {str(e)[:50]}...)"
                
            except Exception as heuristic_error:
                logging.error(f"Heuristic fallback also failed: {heuristic_error}")
                cost = 0
                m = 0
                message = f"Both VRPSolverEasy and heuristic failed: {str(e)[:50]}..."
                formatted_routes = []

        return cost, m, message, routes, exec_time

    def solve_files_in_directory(self,data,cust):
        
        start_time = time.time()
        cost, m, message, routes,ex_time  = self.solve_demo(data,cust,F=1000)
        solve_time = time.time() - start_time
        return cost,m,message,routes,ex_time




