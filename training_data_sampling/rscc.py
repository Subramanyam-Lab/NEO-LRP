...
def sample_lrp_data(lrp_data, num_customers, rng):
    """
    Samples customers uniformly at random and assigns them to a randomly selected depot.
    Checks capacity constraints.
    Returns sampled data if constraints are met, else returns None.
    """
    ...
    if num_customers > len(all_customers):
        logger.warning(f"Requested number of customers ({num_customers}) exceeds available customers ({len(all_customers)}).")
        return None

    sampled_customers = rng.sample(all_customers, num_customers)

    # Randomly select a depot
    depot_idx = rng.randint(0, lrp_data[1] - 1)
    chosen_depot = all_depots[depot_idx]
    depot_capacity = all_depot_capacities[depot_idx]
    total_demand = sum(cust[2] for cust in sampled_customers)

    logger.debug(f"Selected Depot Index: {depot_idx}, Capacity: {depot_capacity}, Total Demand: {total_demand}")
    ...
    if depot_capacity >= total_demand:
        ...
        return None  # Capacity constraint not met
...
def main(folder_path, seed, output_filename_prefix, num_samples, output_folder, default_instance=None):
    """
    Main function to generate VRP instances based on LRP data, ensuring that num_samples unique VRP instances are collected.
    """
    ...
    while len(unique_vrp_instances) < num_samples:
        ...
        # Sample n uniformly from 1 to total_customers
        num_customers_to_sample = rng_main.randint(1, total_customers)
        logger.debug(f"Sampling {num_customers_to_sample} customers uniformly at random.")

        # Sample and assign to depot using the local RNG
        sampled_data = sample_lrp_data(lrp_data, num_customers_to_sample, rng_main)
        ...
        if sampled_data:
            ...
            if vrp_identifier in unique_vrp_instances:
                ...
                output_filename_full = os.path.join(
                    output_folder,
                    f"{output_filename_prefix}_{selected_file_without_extension}_{len(unique_vrp_instances)}_customers_{num_customers_to_sample}_seed_{current_seed}.txt"
                )
            ...
            logger.warning(f"Capacity constraints not met. Skipping to next attempt.")
