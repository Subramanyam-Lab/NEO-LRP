...
Usage:
    # All instances (writes combined Excel directly)
    python run.py --dataset P_prodhon --normalization cost_over_fi --N 110000 --solver vroom --num_runs 5
    
    # Single instance (array job - writes individual Excel)
...
MODELS_DIR = os.path.join(BASE_DIR, "trained_models", "deepsets")

SCALED_DATASETS = ["P_prodhon", "S_schneider"]
UNSCALED_DATASETS = ["T_tuzun", "B_barreto"]


def get_model_paths(dataset, normalization, N):
    if dataset in SCALED_DATASETS:
        model_subdir = "scaled"
    elif dataset in UNSCALED_DATASETS:
        model_subdir = "unscaled"
    else:
        raise ValueError(f"Unknown dataset: {dataset}")
    
    phi_path = os.path.join(MODELS_DIR, model_subdir, "phi", normalization, f"{N}.onnx")
    rho_path = os.path.join(MODELS_DIR, model_subdir, "rho", normalization, f"{N}.onnx")
    return phi_path, rho_path
...
def run_single_instance(config, file_path, phi_loc, rho_loc, normalization, solver_choice, bks_dict):
...
def main():
    ...
    if args.num_runs < 1:
        raise ValueError(f"num_runs must be >= 1")
    ...
    # Get model paths
    phi_loc, rho_loc = get_model_paths(args.dataset, args.normalization, args.N)
    ...
    # Determine mode: single instance (array job) or all instances
    ...
    json_subdir = f"{args.solver}_{args.normalization}_{args.N}"
    plot_subdir = f"{args.solver}_{args.normalization}_{args.N}_plots"
    ...
    if is_array_job:
        excel_temp_dir = os.path.join(base_output_dir, f"{args.solver}_{args.normalization}_{args.N}_excel_temp")
        os.makedirs(excel_temp_dir, exist_ok=True)
    
    all_results = []
    all_ans = {}  # Store ans for each instance
    ...
    for filename in instances:
        ...
        for run in range(args.num_runs):
            ...
            result, ans = run_single_instance(
                ...
                args.normalization, args.solver, bks_dict
            )
    ...
    if is_array_job:
        ...
        rows = []
        for instance_name in instances:  # Preserve order
        ...
        excel_path = os.path.join(
            base_output_dir,
            f"{args.dataset}_{args.N}_{args.normalization}_{args.solver}.xlsx"
        )
