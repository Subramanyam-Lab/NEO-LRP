"""
Graph Transformer Network for Location-Routing Problems
Implements neural network architecture for routing cost prediction
"""
...
class GraphTransformerNetwork(nn.Module):
    """
    Graph Transformer Network for LRP routing cost prediction.
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, heads, beta, dropout, normalization, num_gat_layers, activation, decode_method):
        ...
        # Output layer for final embeddings (φ)
        self.transformer_layers.append(
        ...
        # ρ network: Linear readout for graph-level scalar prediction (route cost)
        self.output_layer = nn.Linear(out_channels, 1)

        self.decoder_mlp = None
        # if decode_method == 'mlp':
        #     # Repeat (Linear -> ReLU) for decoder_num_layers times, then final (Linear -> 1)
        #     if decoder_num_layers < 1:
        #         raise ValueError("decoder_num_layers must be >= 1")
        #     layers = []
        #     in_dim = out_channels
    ...
    def encode(self, data):
        """
        φ network: Computes embeddings for each node through Transformer layers.
        """
        ...
        for i, layer in enumerate(self.transformer_layers):
            ...
            x = self.activation(x)
        # x: Final embedding for each node (φ output)
        return x, batch
    ...
    def forward(self, data, mask=None):
        """
        Complete forward: First computes node embeddings in φ network (encode),
        then aggregates only selected nodes (according to mask) through ρ network (readout) for final prediction.
    ...
    def __repr__(self):
        return (f"{self.__class__.__name__}(in_channels={self.input_transform.in_features}, "
                f"out_channels={self.output_layer.out_features}, heads={self.transformer_layers[0].heads})")