...
def load_config(config_path):
    ...
    # Handle normalization file path (matches config key)
...
def _create_data_json(file_loc):
    ...
    route_cost = int(data.get("vehicle_costs"))
    rc_cal_index = 0
...
def norm_data_gt(cord_set1, cord_set2, veh_cap, cust_dem, rc_cal_index=None, config=None):
    """
    Normalize data for Graph Transformer model.
    
    Difference from norm_data():
    - Inserts depot at row 0 with coordinates (0, 0) and demand 0
    - Returns dict with 'df' (N+1 rows) and 'dist' ((N+1) x (N+1) matrix)
    
    This is required because GT expects per Doyoung's code:
    - Node 0 = depot (0, 0, 0)
    - Nodes 1..N = customers
    """
    ...
    facility_dict = {}
    for j in range(len(cord_set1)):
        if isinstance(veh_cap, (list, np.ndarray)):
            cap = veh_cap[j] if j < len(veh_cap) else veh_cap[0]
        else:
            cap = veh_cap
        
        norm_cust_dem = cust_dem / cap
        coords = mod_coord[j]

        x_vals = np.insert(coords[:, 0], 0, 0.0)  # Depot at origin
        y_vals = np.insert(coords[:, 1], 0, 0.0)
        dem_vals = np.insert(norm_cust_dem, 0, 0.0)  # Depot has 0 demand
        ...
        dist_mtx = scipy_distance_matrix(new_coords, new_coords).astype(np.float32)
